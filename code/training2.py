...
class MyDataSet(Dataset):
    def __init__(self,scoretemp_path,wordpairs,dump=None,k=None):
        if k == None:
            self.word_pairs = wordpairs
...
def predict_prob_countacc(model,wordpairs,templates):
    ...
    for i in range(num_shuffle):
        if i == num_shuffle-1:
            wrong_pair = (random_pairs[i][0],random_pairs[0][1])
            pred_negative = predict_prob(model,wrong_pair,templates)
            if pred_negative==False:
                count+=1
        else:
            wrong_pair = (random_pairs[i][0],random_pairs[i+1][1])
            ...
            if pred_negative==False:
                count+=1  
    '''         
    #return count/(len(wordpairs)*(1+num_shuffle))
...
def predict_max_countacc(model,wordpairs,templates):
    ...
    for i in range(num_shuffle):
        if i == num_shuffle-1:
            wrong_pair = (random_pairs[i][0],random_pairs[0][1])
            pred_negative = predict_max(model,wrong_pair,templates)
            if pred_negative==False:
                count+=1
        else:
            wrong_pair = (random_pairs[i][0],random_pairs[i+1][1])
...
def testtrain():
    scoretemppath = scoretemp_animal_shelter
    wordpairpath = wordpairpath_animal_shelter

    model = Classifier()
    complete_wordpairs = GetPairs(wordpairpath)
    len_dataset = len(complete_wordpairs)
    #random.shuffle(complete_wordpairs)
    train_wordpairs = complete_wordpairs[0:int(split_rate*len_dataset)]
    complete_wordpairs.reverse()
    val_wordpairs = complete_wordpairs[0:int(len_dataset*(1-split_rate))]
    dataset = MyDataSet(scoretemppath,train_wordpairs,k=30)
    ...
    trained_model = train(model, dataloader, torch.optim.Adam(model.parameters(),  lr=0.0003, weight_decay=0.0003), clip=0., num_epochs=5, verbose=True, device='cuda')
    #trained_model = model
